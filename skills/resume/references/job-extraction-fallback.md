...
If `FIRECRAWL_API_KEY` is set in the environment, run this exact `curl`
command via Bash, substituting `<URL>` with the job posting URL:

```bash
curl -s -X POST https://api.firecrawl.dev/v1/scrape \
  -H "Authorization: Bearer $FIRECRAWL_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"url": "<URL>", "formats": ["markdown"], "proxy": "stealth", "waitFor": 8000, "timeout": 55000}'
```
