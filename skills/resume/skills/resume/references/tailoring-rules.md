...
Follow these rules while producing the tailored `ResumeJSON` (schema: the
`ResumeJSON` zod contract in `scripts/validate.mjs`). They are non-negotiable
— violating one produces incorrect output. `scripts/validate.mjs` re-checks
several of these deterministically after you write the JSON (see SKILL.md);
getting them right the first time avoids a correction round, but do not
narrate the checking in your output — just produce output that already
satisfies them.
...
rules. Do not add skills that appear only in the job description. When
ordering the skills array, list skills that also appear in the job posting
first — same alias rules as R1/R10 apply (e.g. the posting saying
"Kubernetes" matches a source skill listed as "K8s"). This is ordering
only, never addition: a skill not demonstrated in the source never appears
here regardless of how prominently the job posting features it.
...
**R12. Job-description keyword coverage (soft, ATS-facing).** Most résumés
tailored well by R1-R11 will already do well here — this rule exists to
catch the residual gap. Applicant tracking systems weight exact or
near-exact token matches with the job posting heavily, and don't reliably
equate synonyms the way a human reader would. When a truthful synonym
choice exists under R10 (e.g. the posting says "Amazon Web Services" and
the source says "AWS"), prefer the posting's exact surface form when it's
equally natural — this is still bounded by R1/R10: never introduce a term
the source doesn't support just to match the posting. Aim to reflect at
least 60% of the job posting's named skills/tools/methodologies somewhere
in the tailored output (summary, skills, or bullets combined), using only
terms the source résumé actually supports.

## Order of operations

1. Read the job posting. Identify the top 5-8 skills, responsibilities, and
   qualifications it emphasizes — these are also what R12's coverage check
   is measured against.
...
4. Write the summary last, grounding it in the final bullet set. Build the
   skills array from source evidence only, ordered per R7.
...
derived-duration, and invented-number rules deterministically — see
SKILL.md. If it reports a violation, fix your JSON directly (you have full
