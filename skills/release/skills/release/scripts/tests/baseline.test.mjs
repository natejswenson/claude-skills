...
// Pinned against a real run of release. Refresh with:
//   node scripts/release.js changelog-draft --from evals/baseline --out <a fresh dir>
//   skillfactory freeze --skill release --from <that dir> --command "node scripts/release.js changelog-draft --from evals/baseline --out $OUT"
...
test('re-running the frozen command reproduces it byte for byte', () => {
  const out = mkdtempSync(join(tmpdir(), 'release-baseline-'));
...
test('the known-bad case still fails', () => {
  ...
  assert.throws(
    () => sh("node scripts/release.js cut --repo . --component press", SKILL),
