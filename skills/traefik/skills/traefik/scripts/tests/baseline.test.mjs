...
// Pinned against a real run of traefik. Refresh with:
//   node scripts/traefik.js check --input evals/capabilities.json --out "<a fresh dir>"
//   skillfactory freeze --skill traefik --from <that dir> --command "node scripts/traefik.js check --input evals/capabilities.json --out \"$OUT\""
...
test('re-running the frozen command reproduces it byte for byte', (t) => {
  const out = mkdtempSync(join(tmpdir(), 'traefik-baseline-'));
...
test('the known-bad case still fails as an incompatible contract', () => {
  const result = spawnSync(process.execPath, ['scripts/traefik.js', 'check', '--input', 'evals/incompatible.json'], { cwd: SKILL, encoding: 'utf8' });
  assert.ifError(result.error);
  assert.equal(result.status, 1);
  assert.equal(JSON.parse(result.stdout).error.code, 'cli_incompatible');
});
