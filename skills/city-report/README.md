...
codex plugin add city-report@claude-skills
```
...
```bash
S="$PWD/skills/city-report/skills/city-report"  # from this repository checkout
...
- **Claude Code:** Allow Python and network access to the public Data USA API.
- **Codex:** Allow the same Python and API access; no connected app or API key is required.
- **Personal data:** Both hosts cache public data in the system temporary directory under `city-report/`; there is no private `~/.claude/` configuration.
