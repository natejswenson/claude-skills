...
// Pinned against a real run of local-dev. Refresh with:
//   node scripts/local-dev.js report --input evals/real-inspection.json --out "<a fresh dir>"
//   skillfactory freeze --skill local-dev --from <that dir> --command "node scripts/local-dev.js report --input evals/real-inspection.json --out \"$OUT\""
...
test('re-running the frozen command reproduces it byte for byte', () => {
  const out = mkdtempSync(join(tmpdir(), 'local-dev-baseline-'));
...
test('the known-bad case still fails', () => {
  ...
  assert.throws(
    () => sh("node scripts/local-dev.js report --input evals/invalid-inspection.json", SKILL),
