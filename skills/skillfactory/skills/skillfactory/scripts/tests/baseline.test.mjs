...
// Pinned against a real run of skillfactory. Refresh with:
//   cp -R evals/inputs/house/. <a fresh dir> && node scripts/skillfactory.js scaffold --spec evals/inputs/demo.spec.json --repo $OUT --today 2026-08-01 >/dev/null
//   skillfactory freeze --skill skillfactory --from <that dir> --command "cp -R evals/inputs/house/. $OUT && node scripts/skillfactory.js scaffold --spec evals/inputs/demo.spec.json --repo $OUT --today 2026-08-01 >/dev/null"
...
test('re-running the frozen command reproduces it byte for byte', () => {
  const out = mkdtempSync(join(tmpdir(), 'skillfactory-baseline-'));
...
test('the known-bad case still fails', () => {
  ...
  assert.throws(
    () => sh("node scripts/skillfactory.js check-spec --spec evals/inputs/degraded.spec.json --repo evals/inputs/house", SKILL),
