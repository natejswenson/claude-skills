 ...
 *   skillfactory    additionally required of skills skillfactory created. The existing skills
 ...
 *
 * The tiers exist so `verify` can be run against ghfactory or devlog and be right
...
/** A skill is skillfactory-native when its invariants say so. Claiming it opts into the stricter tier. */
export const isFactoryNative = (skill) => Boolean(skill.invariants?.skillfactory);

/** Minimum description length skillfactory enforces on skills it creates. */
 ...
 * triggers. skillfactory demands a description that names concrete trigger phrases.
 */
...
export function conform(house, skill) {
  ...
  const out = [];
  const native = isFactoryNative(skill);
  ...
  /**
   * House tier, not skillfactory tier, and deliberately so: every shipped skill was
  ...
  out.push(
    check(
      ...
      'the README house style is in skillfactory/references/readme.md — fixed head, free tail, fixed foot',
    ),
  );

  // ---- skillfactory tier: only asserted against skills that claim it ----------------
...
/** Fatal levels for a given skill: house always, skillfactory only when claimed. */
export function fatalLevels(skill) {
  return isFactoryNative(skill) ? ['house', 'skillfactory'] : ['house'];
}
