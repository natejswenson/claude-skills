...
// ---------------------------------------------------------------------------
// NOT YET FROZEN. repocount has no baseline until it has actually been run.
//
// Run the skill end to end on a real input, then:
//     skillfactory freeze --skill repocount --from <the run's output dir>
//
...
test('a real run has been frozen as the baseline', () => {
  assert.ok(
    ...
    'no real run frozen yet — repocount is at rung 2 (scaffolded and lint-clean), not rung 3. Run it for real, then `skillfactory freeze`.',
  );
});
