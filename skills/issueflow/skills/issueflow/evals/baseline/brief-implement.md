...
Treat the approved Files section as a scope allowlist. Before editing,
check every intended path against it; if the fix needs a new path or
would materially exceed the plan's rough line budget, stop and report it
under Deviations instead of widening the job silently.
