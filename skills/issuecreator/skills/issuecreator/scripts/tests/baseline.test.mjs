...
// Pinned against a real run of issuecreator. Refresh with:
//   node scripts/issuecreator.js render --input evals/real-draft.json --out "<a fresh dir>"
//   skillfactory freeze --skill issuecreator --from <that dir> --command "node scripts/issuecreator.js render --input evals/real-draft.json --out \"$OUT\""
...
test('re-running the frozen command reproduces it byte for byte', () => {
  const out = mkdtempSync(join(tmpdir(), 'issuecreator-baseline-'));
...
test('the known-bad case still fails', () => {
  ...
  assert.throws(
    () => sh("node scripts/issuecreator.js validate --input evals/incomplete-draft.json", SKILL),
