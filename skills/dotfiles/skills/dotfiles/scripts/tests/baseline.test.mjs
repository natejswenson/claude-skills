...
// Pinned against a real run of dotfiles. Refresh with:
//   node evals/replay.mjs "<a fresh dir>"
//   skillfactory freeze --skill dotfiles --from <that dir> --command "node evals/replay.mjs \"$OUT\""
...
test('re-running the frozen command reproduces it byte for byte', () => {
  const out = mkdtempSync(join(tmpdir(), 'dotfiles-baseline-'));
