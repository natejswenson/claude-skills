...
// Pinned against a real run of eval. Refresh with:
//   node scripts/eval.js report --contract evals/fixtures/contracts/skillfactory.json --trace evals/fixtures/run-trace.json --skill skillfactory --out <a fresh dir>
//   skillfactory freeze --skill eval --from <that dir> --command "node scripts/eval.js report --contract evals/fixtures/contracts/skillfactory.json --trace evals/fixtures/run-trace.json --skill skillfactory --out $OUT"
...
test('re-running the frozen command reproduces it byte for byte', () => {
  const out = mkdtempSync(join(tmpdir(), 'eval-baseline-'));
...
test('the known-bad case still fails', () => {
  ...
  assert.throws(
    () => sh("node scripts/eval.js probe --contract evals/fixtures/contracts/skillfactory.json --trace evals/fixtures/run-trace.json --check-finding evals/fixtures/bad-finding.json", SKILL),
