 ...
 *
 * This is smith's one rule made mechanical. A skill is not done when its files
...
//   smith freeze --skill ${spec.name} --from <that dir> --command ${JSON.stringify(manifest.command)}
...
${
  ...
  //     smith freeze --skill ${spec.name} … --trap-command "<a command that must exit non-zero>"
  // ---------------------------------------------------------------------
