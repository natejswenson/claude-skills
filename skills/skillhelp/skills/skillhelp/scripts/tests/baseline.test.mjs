...
// Pinned against a real run of skillhelp. Refresh with:
//   node scripts/skillhelp.js build --repo evals/fixtures/snapshot --skill-dir <a fresh dir>
//   skillfactory freeze --skill skillhelp --from <that dir> --command "node scripts/skillhelp.js build --repo evals/fixtures/snapshot --skill-dir $OUT"
...
test('re-running the frozen command reproduces it byte for byte', () => {
  const out = mkdtempSync(join(tmpdir(), 'skillhelp-baseline-'));
