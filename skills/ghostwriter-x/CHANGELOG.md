...
- **Every opted-in Codex-generated card receives a recorded visual review before presentation or attachment.** All 12 dimensions must pass after full-resolution and feed-size inspection, including post alignment, visual appeal, PRESS brand, factual relationships, exact copy, readability and accessibility. Reviews bind the exact image, target tweet, thread, prompt, brand and evidence; changed or stale inputs require another review. Native screenshots and Claude/legacy renders retain their existing workflow.
- **Optional shared hashtag memory preserves the X voice source.** An explicitly configured owner bridge can recall and save X-specific hashtag preferences with verified writes, scoped access and retry handling. Installation does not enable memory, migrate preferences or share them across platforms.

### Changed

- **Existing drafts need a current editorial review before display, Typefully draft creation or publication.** Ten checks cover voice, naturalness, substance, clarity, opening, ending, credibility, restraint, originality and platform fit. The editor compares a direct opening alternative and tests cutting or tightening the weakest passage; unresolved warnings, stale evidence and older review records block delivery until reviewed again.
...
- Corrected generated-region and version handling.
