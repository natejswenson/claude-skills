...
2. **Pick 1–2 body components** from the vocabulary below (between the masthead and
...
|---|---|---|---|
| **Ledger** | `.ledger > .lrow` (`.lno .lbody .lt .le`) | a method, ≤3 steps (landscape) | `.lt` ≤48 · `.le` ≤70 |
...
| **Tiles** | `.tiles > .tile` (`.tno .tt .te`) | exactly 4 compact steps, one row across | `.tt` ≤22/line |
| **Terminal** | `.term > .tl` (+ `.prompt`, `.dim`, `.hot`) | real code / a session | accent ≤8 rows ≤42 chars · hero ≤12 rows ≤64 chars |
...
2. **Headline treatment** — sig-phrase placement (start vs end), full-size vs `compact`,
   1-line vs 2-line.
3. **Density** — airy (1 body component) vs packed (2).
...
ink rules automatically. The X blueprint is a 4-image post (4 slides, ≤30 words/slide,
