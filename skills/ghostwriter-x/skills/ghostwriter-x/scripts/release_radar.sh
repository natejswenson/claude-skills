#!/usr/bin/env bash
#
# Release Radar — twice-weekly research run for the X ghostwriter.
#
...
# does RESEARCH ONLY — it never posts to X (research only, by design; see COMPLIANCE.md).
#
# Invoked by the launchd agent com.nate.x-release-radar, or run manually:
#     bash scripts/release_radar.sh
#
set -uo pipefail

RADAR_TRUSTED="$HOME/.claude/ghostwriter-x/radar/trusted"
...
if [[ $STATUS -eq 0 && -f "$REPO/$DIGEST" ]]; then
  ...
  osascript -e "display notification \"New digest: ${DIGEST}. Say 'draft a post from item N in the radar'.\" with title \"X Release Radar\"" >/dev/null 2>&1
else
  log "ERROR: run exited $STATUS or digest not written ($DIGEST missing)."
  osascript -e "display notification \"Release radar run failed (exit ${STATUS}). Check research/.radar.log.\" with title \"X Release Radar\"" >/dev/null 2>&1
  exit 1
fi
