#!/usr/bin/env python3
"""Render an X visual (Mermaid diagram or HTML card) to a high-DPI PNG.

Cards are landscape 16:9 (1200×675) — X's native timeline crop. This is the
OPTIONAL diagram feature. It needs Playwright + Chromium:
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Viewport WxH hint only — the screenshot crops to #canvas, which sizes "
             "itself from CSS. Default 1200x675 (X's landscape 16:9).",
    )
    ap.add_argument(
        "--no-open",
        action="store_true",
    ...
    try:
        ...
        sys.exit(f"ERROR: --size must look like 1200x675, got {args.size!r}")
