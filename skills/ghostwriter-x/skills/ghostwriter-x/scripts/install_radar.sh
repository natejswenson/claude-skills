#!/usr/bin/env bash
#
# Install (or repair) the release-radar launchd agent (X).
#
...
#
# It also unloads + removes any OTHER "*x-release-radar*" agents found in
...
for old in "$AGENTS_DIR"/*x-release-radar*.plist; do
...
sed -e "s|REPO_PATH|$REPO|g" \
    -e "s|com.example.x-release-radar|$LABEL|g" \
    "$TEMPLATE" >"$PLIST"
