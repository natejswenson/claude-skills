...
def pick_record(records: list[dict], tweet_id: str | None, latest: bool) -> dict:
    if tweet_id:
        for rec in records:
    ...
    sys.exit("ERROR: pass --id <tweet id> or --latest.")
...
def main() -> None:
    ...
    rec = pick_record(records, args.id, args.latest)
