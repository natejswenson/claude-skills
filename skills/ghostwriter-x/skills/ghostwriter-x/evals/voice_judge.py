"""Tier-3 voice-fidelity judge for ghostwriter-x (on-demand, cost-capped).
...
def _llm_score(text, model, budget):  # pragma: no cover - live judge; not in CI
    ...
    prompt = (
        "You are scoring an X post or thread draft for fidelity to the author's voice. "
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="ghostwriter-x voice-fidelity judge")
