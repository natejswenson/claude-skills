...
Three layers, mirroring the module:
  ...
  * real DOM checks — rendered in actual Chromium against the shipped
    diagram.css.example (landscape 1200×675); skipped cleanly when Chromium
    is missing.
"""
...
def test_default_icon_with_comment_fails():
...
def test_term_width_budget_warns_past_landscape_cap():
    rows = ["x" * (cl.BUDGETS["term"]["max_chars"] + 1)]
    findings = cl.static_checks(term_card(rows), "images/x.html")
...
# -------------------------------------------------------------- carousel checks
def test_carousel_clean_four_slides():
    assert cl.static_checks(carousel(4), "images/x-carousel.html") == []


def test_carousel_n_mismatch_fails():
    findings = cl.static_checks(carousel(4, n_attr=9), "images/x-carousel.html")
    assert codes(findings, "FAIL") == ["carousel-sync"]
    assert "--n:9" in findings[0].message


def test_carousel_i_out_of_order_fails():
    findings = cl.static_checks(carousel(4, i_attr=3), "images/x-carousel.html")
...
def test_carousel_missing_counters_fails():
    html = carousel(4).replace('style="--i:3; --n:4"', "")
...
def test_carousel_pageno_text_mismatch_fails():
    findings = cl.static_checks(
        carousel(4, pageno="<b>02</b> / 09"), "images/x-carousel.html"
    )
    assert codes(findings, "FAIL") == ["carousel-sync"]
    assert '"02 / 04"' in findings[0].message


def test_carousel_slide_count_past_four_warns():
    findings = cl.static_checks(carousel(6), "images/x-carousel.html")
    assert codes(findings, "WARN") == ["slide-count"]
    assert not codes(findings, "FAIL")
    assert "--allow-many" in findings[0].message


def test_carousel_single_slide_warns():
    findings = cl.static_checks(carousel(1), "images/x-carousel.html")
    assert codes(findings, "WARN") == ["slide-count"]


def test_carousel_wordy_slide_warns():
    wordy = carousel(4).replace(
...
def test_cli_carousel_static_only_exit_2(tmp_path, capsys):
    ...
    src.write_text(carousel(4, n_attr=9), encoding="utf-8")
    with pytest.raises(SystemExit) as e:
...
def test_cli_json_output(tmp_path, capsys):
    src = tmp_path / "c-carousel.html"
    src.write_text(carousel(6), encoding="utf-8")  # WARN-only -> exit 1
    with pytest.raises(SystemExit) as e:
...
def test_dom_empty_band_fails_on_sparse_card(dom_page):
    html = card(
        TOPROW + "<h1>Short.</h1>"
        + '<div class="stack">' + sstep("One", "tiny") + "</div>",
        cls="card howto-stack light",
        style=(
            "<style>"
            ".card.howto-stack.light .stack{margin:0 !important; flex:0 0 auto !important;"
            " gap:8px !important}"
            ".card.howto-stack.light .st{font-size:18px !important}"
            ".card.howto-stack.light .sstep{padding:2px 0 !important; min-height:0 !important}"
            ".card.howto-stack.light .sstep::before{font-size:20px !important}"
            "</style>"
        ),
    )
    findings = run_dom(dom_page, html)
    assert "empty-band" in codes(findings)


def test_dom_long_cmd_wraps_instead_of_ellipsizing(dom_page):
    long_cmd = (
        "npx -y @natjswenson/shipflow@latest apply --repo . --branch dev "
        "--merge-method merge --cleanup --and-more --flags-to-force-a-wrap "
        "--really-long-tail-so-it-wraps-even-on-the-wide-landscape-canvas"
    )
    html = card(
...
def test_dom_clip_overflow_fails_on_offcanvas_element(dom_page):
    html = card(
        TOPROW + "<h1>How to do the thing.</h1>"
        + '<div class="steps">'
        + step("One", "detail") + step("Two", "detail") + step("Three", "detail")
        + "</div>" + BAND + CAPTION
        + '<div style="position:absolute; top:800px; left:100px">pushed off</div>',
    )
    findings = run_dom(dom_page, html)
...
def test_dom_count_budget_fails_on_six_steps(dom_page):
    ...
    findings = run_dom(dom_page, html)
    assert any(
        f.code == "count-budget" and "6 × .step" in f.message
