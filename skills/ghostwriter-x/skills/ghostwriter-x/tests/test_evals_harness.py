...
def test_detect_weighted_length_check():
    assert re_mod._did_validate_weighted_length(
        [_bash("python3 scripts/x_len.py --file drafts/x.md --thread")]
    )
    # Reading the script (or just talking about 280) is not measuring.
    assert not re_mod._did_validate_weighted_length([_bash("cat scripts/x_len.py")])


def test_detect_decline_text_and_publish_short_circuit():
    declined = [{"type": "text", "text": "I'll decline — unattended posting needs your approval per post."}]
...
def test_decline_false_when_scheduler_built():
    ...
    assert not re_mod._did_decline_unattended(events)
...
def test_classify_must_and_must_not():
    ...
    xlen_events = re_mod._mock_events({"expect": {"must": "validates_weighted_length"}})
    assert re_mod.classify(xlen_events, {"must": "validates_weighted_length"})
