...
landscape sizing." That was previously only caught by a human eyeballing a render.
...
personal `~/.claude/ghostwriter-x/assets/diagram.css` is gitignored and per-user;
the `.example` is the shipped source of truth and the two are kept in sync by
hand — see SKILL.md's Visuals section). Cards are landscape 1200×675 and that is
...
# The carousel is a multi-image set rendered by render_carousel.py (many
...
def test_every_template_header_has_content_budget(tpl: Path):
    ...
    how much content fits the 1200×675 frame."""
...
def test_howto_family_is_registered_and_synced():
    """The how-to family is the how-to family: every variant must have a
