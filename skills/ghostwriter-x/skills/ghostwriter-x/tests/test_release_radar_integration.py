...
GHOSTWRITER_X_RADAR_INTEGRATION=1 pytest tests/test_release_radar_integration.py --no-cov -s
Optional GHOSTWRITER_X_RADAR_PROBE_DIR retains artifacts outside pytest's temp tree.
...
pytestmark = pytest.mark.skipif(os.environ.get("GHOSTWRITER_X_RADAR_INTEGRATION") != "1",
...
def test_real_codex_policy_tamper_receipts_and_next_launch(tmp_path):
    ...
    base = Path(os.environ.get("GHOSTWRITER_X_RADAR_PROBE_DIR", tmp_path)).resolve() / uuid.uuid4().hex
    ...
    try:
        config = radar.install(bundle, home, interests=interests, executable=executable,
                               codex_home=auth, label="com.example.x-radar-probe-" + base.name,
                               load_agent=True)
