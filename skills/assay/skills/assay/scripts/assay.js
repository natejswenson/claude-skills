#!/usr/bin/env node
/**
 * assay — the deterministic half of the skill.
 *
...
async function cmdCase(args) {
  ...
  const detail = args.detail ?? 'converted from an assay finding';
...
const USAGE = `assay v${VERSION} — grade a real run of a skill against the contract that skill committed to.
  ...
  assay case     --skill <name> --in <file> --assert-absent <text> --prove [--finding <id>]
`;
