...
const cli = (args) => execFileSync('node', [join(SKILL, 'scripts', 'assay.js'), ...args], { cwd: SKILL, encoding: 'utf8', stdio: 'pipe' });
...
test('a case that passes on arrival is refused and deleted', () => {
  ...
  let output = '';
  try {
    output = cli(['case', '--skill', 'assay', '--repo', REPO, '--in', 'SKILL.md', '--prove', '--finding', 'trap-green-case', '--assert-absent', ABSENT]);
  } catch (err) {
    failed = true;
...
test('a case generated without --prove is refused', () => {
  assert.throws(
    () => cli(['case', '--skill', 'assay', '--repo', REPO, '--in', 'SKILL.md', '--assert-absent', ABSENT]),
...
test('a case that IS red today is kept', () => {
  ...
  try {
    const out = cli(['case', '--skill', 'assay', '--repo', REPO, '--in', 'SKILL.md', '--prove', '--finding', id, '--assert-absent', present]);
