...
// Pinned against a real run of assay. Refresh with:
//   node scripts/assay.js report --contract evals/fixtures/contracts/smith.json --trace evals/fixtures/run-trace.json --skill smith --out <a fresh dir>
//   smith freeze --skill assay --from <that dir> --command "node scripts/assay.js report --contract evals/fixtures/contracts/smith.json --trace evals/fixtures/run-trace.json --skill smith --out $OUT"
...
test('re-running the frozen command reproduces it byte for byte', () => {
  const out = mkdtempSync(join(tmpdir(), 'assay-baseline-'));
...
test('the known-bad case still fails', () => {
  ...
  assert.throws(
    () => sh("node scripts/assay.js probe --contract evals/fixtures/contracts/smith.json --trace evals/fixtures/run-trace.json --check-finding evals/fixtures/bad-finding.json", SKILL),
