 ...
 */
/*
 * NOTE THE MISSING \b. Domains concatenate words — "sanfordhealth.org",
 ...
 * into a folder called "Onlinejmc" after its mail vendor.
 *
 ...
 * is different, and being too strict has its own cost: `@onecallcm.com` fails
 * an exact match against `Recruiting/One Call` (the token is `onecallcm`, the
 ...
 * shorter side. `onecall` ⊂ `onecallcm` matches; `uhg` against
 * `unitedhealthgroup` does not, and should not — nothing in that address says
 * UnitedHealth Group, and naming it is the user's call.
 */
 ...
 * under `Onlinejmc`, and the same failure the folder split was supposed to fix.
 *
  ...
  for (const t of threads) {
    ...
    for (const r of actors) {
      ...
      taken.push({
        ...
        // `Recruiting/One Call` to mail already carrying `Recruiting` must not
...
export const buildReceipt = (entries, { at }) => ({
  ...
  entries: entries.map((e) => ({
    ...
    // the labels the thread ends up with. Filing into `Recruiting/One Call`
