...
The workflow enables native auto-merge only for ready main PRs whose head repository
matches this repository. Drafts skip; `ready_for_review` enables automation when a
reviewed draft becomes ready. Repository write access is the trust boundary. Forks
and missing head repositories skip both jobs. Maintainers review fork PRs and
explicitly enable native auto-merge or merge using their own authorized credentials
after required checks; they may add `release-pending` manually. The workflow keeps
`pull_request` and executes no PR code.

Merged same-repository PRs receive `release-pending`; unmerged closes skip. These
labels are optional reminders: component release-status discovers untagged work
without them. No merge creates a tag or release. Both jobs require the configured
PAT/App secret; an unavailable token produces an explicit clean skip, with no
fallback to `GITHUB_TOKEN`. Provisioning that secret remains a separate setup step.

For a template migration, the recorded old render hash must match the existing
workflow. Plan/apply then recognizes a generator update; commit the generated YAML
and returned hash receipt together. A genuine hand edit still blocks apply. Use the
corrected checkout below while this change is unreleased.
...
| local-dev | a real offline inspection of its development worktree, with only the machine path replaced; the same snapshot with an inconsistent dirty flag; temporary Git repositories and recording GitHub CLI doubles | inspection accidentally accessing the network or changing Git state, lost dirty/rename paths, missing custom input, unsafe issue arguments, and invalid evidence rendered as a valid report; model workflow behavior is separately forward-tested |
| traefik | a real external `tk capabilities --json` response and the helper compatibility projection, replayed offline; the same response with removal support removed | an older CLI accepted by a newer skill, stack discovery depending on a plugin cache path, or private installation paths leaking into the baseline |
