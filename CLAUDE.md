...
- `main` required checks — **one per skill, no exceptions**: `ci / devlog`, `ci / resume`,
  ...
  `ci / pluginsync`, `ci / issueflow`, `ci / shipreport`, `ci / gmailtriage`, `ci / skillhelp`, `ci / brandreport`, `ci / netwatch`, `ci / appletv`. These names are the job `name:` values — **renaming a caller or its `ci`
