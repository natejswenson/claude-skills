...
| gmailtriage | two real runs against this account's own mailbox — a 60-thread inbox triage, and a retroactive pass splitting its `Recruiting` folder into four sub-labels (13 threads, two of them behind applicant-tracking vendors) — `propose`, `subdivide`, `plan` and `apply` re-run and byte-compared; plus a second retroactive plan that must take **zero** | a sub-label rule silently ceasing to apply its parent, so a folder's contents come to depend on when each rule was written; a cluster being auto-named after the mail vendor that hosts it rather than the organisation, which is the school-district-under-its-mail-vendor bug committed by the fix for it; a broad rule standing in front of a rule filing into a sub-label of it, which does not fail but splits the mailbox by arrival time; a retroactive pass ceasing to converge, so every run re-files the same mail forever while every table still reads correctly; and a redaction that pseudonymises senders but merges them, so `subdivide` sees one cluster and reports "still one thing" over a corpus that no longer discriminates |
