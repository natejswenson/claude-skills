...
| gmailtriage | three real runs against this account's own mailbox — a 60-thread inbox triage; a retroactive pass splitting its `Recruiting` folder into four sub-labels (two of them behind applicant-tracking vendors); and a label cleanup, frozen as the mailbox **before** it (5 unmanaged folders, a transposed duplicate pair, 15 unclaimed threads, 74% coverage) and **after** it (100%, clean, exit 0) — with `propose`, `subdivide`, `plan`, `apply`, `audit` and `merge` re-run and byte-compared, plus a retroactive re-plan that must take **zero** | a sub-label rule ceasing to apply its parent, so a folder's contents come to depend on when each rule was written; a cluster auto-named after the mail vendor hosting it rather than the organisation; a broad rule in front of a rule filing into a sub-label of it, which does not fail but splits the mailbox by arrival time; a retroactive pass ceasing to converge; the audit answering `plan`'s question instead of its own, which reported 47 of 48 correctly-filed threads as unclaimed; the clean state ceasing to be reachable, so the gate could never be satisfied; a merge unlabelling before it labels, leaving mail in neither folder; and a redaction that pseudonymises senders but merges them, or drops thread counts, so the corpus passes over a signal that no longer discriminates |
